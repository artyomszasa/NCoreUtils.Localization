...
type JsonResourcesTests () =

  let services =
    ServiceCollection()
      .AddLogging()
      .AddSourceBasedLocalization(fun b -> b.AddEmbeddedJson(typeof<JsonResourcesTests>.Assembly, "i18n") |> ignore)
  ...
  member this.TypedStringLocalizerTests () =
    ...
    serviceProvider.GetRequiredService<IStringLocalizer<Shared>>() |> this.TestLocalizer
    // serviceProvider.GetRequiredService<IStringLocalizerFactory>().Create(null, "Inner") |> this.TestLocalizer
    serviceProvider.GetRequiredService<IStringLocalizerFactory>().Create("A", "NCoreUtils.Localization.Unit.i18n.Inner") |> this.TestLocalizer
